...
unsigned char DotMatrix_TableSearch(unsigned char SearchLetter)
{
  ...
  /*
    if(SearchLetter >='A' && SearchLetter <='Z') 
        return (SearchLetter - 'A' + 10);
    else if(SearchLetter >='a' && SearchLetter <='z') 
        return (SearchLetter - 'a' + 36);
    else if(SearchLetter >='0' && SearchLetter <='9') 
        return (SearchLetter - '0');
    else if(SearchLetter >= 160 && SearchLetter <= 215) 
        return (SearchLetter - 160 + 62);
    else if(SearchLetter == 216 || SearchLetter == 217) 
        return (SearchLetter - 88);
                ...
                for(j = 0; j < Letter_Width; j++)
                {
                    switch(Motion)
                    {
                        ...
                        case Show_MiddleFixed:  
                            ...
                            {
                                DotMatrix_Window[DotCheck] = LetterTable[DotMatrix_TableSearch(*(String + i))][j];
                            }
                            break;
                        case Show_UpToDown:
                            DotCheck = i*Letter_Width + j + Window_ShiftDots;
                            ...
                            {
                                    if(DotMovedNumber_String <= Letter_Height + 1)
                                        DotMatrix_Window[DotCheck] = ((LetterTable[DotMatrix_TableSearch(*(String + i))][j]) & DotMask) >> (Letter_Height + 1 - DotMovedNumber_String);
                                    else
                                        DotMatrix_Window[DotCheck] = ((LetterTable[DotMatrix_TableSearch(*(String + i))][j]) & DotMask) << (DotMovedNumber_String - Letter_Height - 1);
                            }
                            break;
                        case Show_DownToUp:
                            DotCheck = i*Letter_Width + j + Window_ShiftDots;
                        ...
                        case Show_RightToLeft:
                                if((j + DotMovedNumber_String) < Letter_Width)
                                    DotMatrix_Window[i*Letter_Width + j + RemainingDot] = LetterTable[DotMatrix_TableSearch(*(String + i))][j + DotMovedNumber_String];
                                else
                                    DotMatrix_Window[i*Letter_Width + j + RemainingDot] = LetterTable[DotMatrix_TableSearch(*(String + i + 1))][j + DotMovedNumber_String - Letter_Width];
                            break;     
                    }
                }
            }
...
{
    NumberInfo SelWindows[_8x4_Windows_Number] = 
    {
      {9*LetterWidth_8x4,  4,0,2},    // 1
      {6*LetterWidth_8x4,  3,0,0},    // 2
      {1*LetterWidth_8x4,  4,1,0},    // 3
      {14*LetterWidth_8x4, 4,0,1},    // 4
      {16*LetterWidth_8x4, 3,0,0},    // 5
      {10*LetterWidth_8x4, 3,0,1},    // 6
      {5*LetterWidth_8x4,  3,0,1},    // 7
      {14*LetterWidth_8x4, 3,0,0},    // 8
      {1*LetterWidth_8x4,  3,0,0},    // 9
      {14*LetterWidth_8x4, 4,0,0},    // 10
    };
    DotMatrix_Number(GroupNumber-1+_8x5_Windows_Number, SelWindows[GroupNumber-1].w_BeginningDot, SelWindows[GroupNumber-1].w_Digits, 
    SelWindows[GroupNumber-1].w_IsTime, SelWindows[GroupNumber-1].w_DecimalPoint, Letter_8x4, NumberData, Motion);
}
*/
//========================================================================================================================
